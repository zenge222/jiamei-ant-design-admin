    ...
    data () {
      return {
        ...
        validatorRules:{
          ...
          code:{rule: [{ required: true, message: '请输入验证码!'}]},
    ...
    methods: {
      ...
      handleSubmit () {
        ...
        if (that.customActiveKey === 'tab1') {
          ...
          that.form.validateFields([ 'mobile', 'code', 'rememberMe' ], { force: true }, (err, values) => {
            if (!err) {
        ...
        this.form.validateFields([ 'mobile' ], { force: true },(err,values) => {
            if(!values.mobile){
              ...
              let smsParams = {};
                  ...
                  smsParams.smsmode="0";
              postAction("/system/sms",smsParams)
                .then(res => {
                  if(!res.code == 0){
                    setTimeout(hide, 0);
      ...
      handleChangeCheckCode(){
        this.currdatetime = new Date().getTime();
        getAction(`/system/randomImage/${this.currdatetime}`).then(res=>{
          if(res.code == 0){
