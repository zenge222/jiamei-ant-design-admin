  ...
  export default {
    ...
    data() {
      return {
        ...
        validatorRules: {
          code: {rule: [{required: true, message: '请输入短信验证码!'}, {validator: this.validateCaptcha}]},
    ...
    methods: {
          ...
          if (!err) {
            if (that.dropList == "0") {
              if (values.code == undefined) {
                ...
                postAction("/system/user/phoneVerification", params).then((res) => {
                  if (res.code == 0) {
                    console.log(res);
                    var userList = {
      ...
      getCaptcha(e) {
        ...
        postAction("/system/sms", smsParams).then(res => {
          if (!res.code == 0) {
            setTimeout(hide, 1);
            this.cmsFailed(res.message);
          }
