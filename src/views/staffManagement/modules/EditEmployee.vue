        ...
        <a-form-model
            ...
            <a-row :gutter="[16,16]">
                <a-col :span="12">
                    ...
                    <a-form-model-item label="详细地址" prop="detailsAddress">
                ...
                <a-col :span="12">
                    ...
                    <a-form-model-item label="员工状态" prop="status">
                        <a-radio-group v-model="formPartOne.status">
                            <a-radio :value="0">
                                离职
                            </a-radio>
                            <a-radio :value="1">
        ...
        <a-form-model
            ...
            <a-form-model-item label="培训经历" :label-col="{span:2}" :wrapper-col="{span:22}" prop="trainingRecordList">
                ...
                <a-row type="flex" style="margin-bottom:10px" :gutter="16" v-for="(item, index) in formPartFour.trainingRecordList" :key="index">
                    ...
                    <a-col :span="12">
                        <a-form-model-item>
                            <a-input v-model="formPartFour.trainingRecordList[index].description" placeholder="例：培训地点/培训内容"/>
                        </a-form-model-item>
                    </a-col>
                    <a-col :span="4">
                        <a-form-model-item>
...
<script>
    ...
    export default {
        name: 'EditEmployees',
        components:{JAreaLinkage,JImageUpload,JEditor,UploadImage},
        data(){
            return{
                companyList:[],
                title:1,//1代表添加、2代表编辑
                ...
                rules:{
                    name:[
                        ...
                        { min: 2, message: '姓名格式错误！', trigger: 'blur' },
                    ],
        ...
        methods:{
            add(){
                this.$refs.formPartOne.resetFields()
                console.log(this.formPartOne)
            },
            submitOnePart(){
                let that = this
                that.formPartOne.idcardAddress = that.areaData.getText(that.formPartOne.idcardAddressCode)+'.'+that.detailsAddress
                if(that.detailsAddress == ''||that.detailsAddress == undefined || that.detailsAddress == null){
                    that.$message.error('详细地址不能为空！')
                    return false
                }
                console.log(that.formPartOne)
                this.$refs.formPartOne.validate(valid =>{
                    if(valid){
                        editEmployees(that.formPartOne).then(res=>{
                            if(res.code ==0){
                                console.log(res)
            ...
            editEmployees(data){
                ...
                getEmployeesMessageInfo(data.id).then(res=>{
                    if(res.code == 0){
                        this.formPartOne = res.data.baseInfo
                        this.getCompanyList(res.data.storeInfo.recruit)
                        this.formPartOne.stature = parseInt(res.data.baseInfo.stature)
                        this.formPartThree = res.data.storeInfo
                        this.formPartThree.restDay = res.data.storeInfo.restDay?res.data.storeInfo.restDay.split(','):[]
                        this.detailsAddress = res.data.baseInfo.idcardAddress.split('.')[1]
                        this.formPartFour.employeeId = res.data.baseInfo.id
                        this.formPartFour.employeeName =res.data.baseInfo.name
                        this.formPartFour.intro = res.data.baseInfo.intro
                        this.formPartFour.jobRecordList = res.data.jobRecordList.length>0?res.data.jobRecordList:[{beginYearMonth:'',endYearMonth:'',company:'',post:'',leaveReason:''}]
                        this.formPartFour.speciality = res.data.baseInfo.speciality?res.data.baseInfo.speciality.split(','):[]
                        this.formPartFour.trainingRecordList = res.data.trainingRecordList.length>0?res.data.trainingRecordList:[{beginYearMonth:'',endYearMonth:'',description:''}]
                        this.formPartFour.workYears = res.data.baseInfo.workYears
                        if(res.data.baseInfo.avatar !=''&&res.data.baseInfo.avatar !=undefined&&res.data.baseInfo.avatar!=null){
                            let obj = {
                                uid:uuid(),
                                name: 'XXXX.png',
                                status: 'done',
                                url:res.data.baseInfo.avatar
                            }
                            this.files.push(obj)
                        }

                        if(res.data.familyList.length>0){
                            this.formPartOne.familyList = res.data.familyList
                        }else {
                            this.formPartOne.familyList = [{ familyName:'',familyPhone:"",familyRelation:'' }]
                        }
                    }else{
                        this.$message.error(res.message)
                    }
            ...
            submitTwoPart(){
                ...
                this.$refs.formPartThree.validate((valid)=>{
                    if(valid){
                        ...
                        editEmployeesStoreInfo(this.formPartThree).then(res=>{
                            if (res.code == 0){
                                this.$message.success('编辑成功！')
                            }else {
                                this.$message.error(res.message)
            ...
            getCompanyList(value){
                if (value ==1) {
                    ...
                    getRecommendListType(1).then(res=>{
                        if(res.code ==0){
                            this.companyList = res.data
                        }
                    })
                }else{
                    getRecommendListType(2).then(res=>{
                        if(res.code ==0){
                            this.companyList = res.data
                        }
                    })
            ...
            chooseRecruit(value){
                this.formPartThree.recruitId = null
                var type;
                console.log(value)
                if(value == 2){
                    type = 1
                ...
                getRecommendListType(type).then(res=>{
                    if(res.code ==0){
                        console.log(res)
                        this.companyList = res.data
                        this.formPartThree.recruitId = res.data.length>0?res.data[0].id:null
                    }else {
                        this.$message.error(res.message)
                    }
                })
