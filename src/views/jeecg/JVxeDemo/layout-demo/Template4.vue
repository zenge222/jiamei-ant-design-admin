    ...
    methods: {
      ...
      loadTable1Data() {
        ...
        getAction(this.url.getData, formData).then(res => {
          if (res.code == 0) {
            // 后台查询回来的 total，数据总数量
            this.table1.pagination.total = res.data.total
            // 将查询的数据赋值给 dataSource
            this.table1.dataSource = res.data.records
            // 重置选择
      ...
      loadTable2Data() {
        ...
        getAction(this.url.getData, formData).then(res => {
          if (res.code == 0) {
            ...
            this.table2.dataSource = res.data.records
            this.table2.selectedRows = []
          } else {
      ...
      loadTable4Data() {
        ...
        getAction(this.url.getData, formData).then(res => {
          if (res.code == 0) {
            this.table4.pagination.total = res.data.total
            this.table4.dataSource = res.data.records
            this.table4.selectedRows = []
          } else {
            this.$error({title: '子表查询失败', content: res.message})
          }
